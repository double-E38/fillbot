...
def setup():
    ...
    print("You chose ", group, group.group_id)

    # Adds user targets.
    get_target = []
    users = group.members()
    while user_option != 'No':
        # See if another target is required. If 'Yes', append target list with
        # target.
        user_option = input("Do you want to add a target? (Yes or No): ")
        if user_option == 'Yes':
            i = 0
            for u in users:
...
def quadzBlock(payload):
    if int(payload['id']) != botParameters.msg_hist:
        ...
        if botParameters.msg_hist_user[0] == botParameters.msg_hist_user[1] and \
                botParameters.msg_hist_user[1] == botParameters.msg_hist_user[2]:
...
def silenceCommand(command, user, id):
    if command == '/silence':
        for key, value in admins.items():
            ...
            if value == id:
                ...
                return
        if botParameters.silence:
            fillbot.post("YOU DIDN'T SAY THE MAGIC WORD!",
                         attachments.Image('https://media.giphy.com/media/5ftsmLIqktHQA/giphy.gif'))
...
def executeCommands(command, user, id):
    if command == '/bad and boujee':
        ...
        if id != useful_users['bebe']:
            ...
            temp_img = attachments.Image(
                'https://i.groupme.com/320x240.gif.9447c57af1224aac910cf77c26ee421d')
            fillbot.post('', temp_img)
            return '/eatshit DENINED'
    elif command == '/analysis':
        temp_img = attachments.Image(
            'https://i.groupme.com/245x245.gif.550f59867ae4403598936c186b8378e1.large')
        fillbot.post('', temp_img)
        return '/analysis'
...
def runTheTrap():
    ...
    if data and not botParameters.silence:
        ...
        if data['sender_type'] != 'bot' and data['sender_type'] != 'system':
            ...
            if int(data['sender_id']) == useful_users['bebe'] and botParameters.eatshit:
                sendMention(["eat shit @", data['name']], data['sender_id'])
            ...
            for a in data['attachments']:
                ...
                if data_attachments['type'] == 'mentions':
                    for u in data_attachments['user_ids']:
                        if int(u) == useful_users['tony']:
                            print("Printed drake from mention")
                            lyrics = random.choice(drizzy_lyrics)
                            fillbot.post(lyrics)
                            break
...
useful_users = {'fillup': 7430058, 'addison': 3359289, 'bebe': 6011409, 'trey': 6606644, 'damn': 6616638,
                'jeff': 963630, 'seth': 9136487, 'rt': 6357297, 'kersten': 6606645, 'nik': 6606860, 'tony': 4256997}
...
bot_group, users, targets = setup()
bots = groupy.Bot.list()
print(bots)
user_option = chooseTarget(len(bots))
fillbot = bots[user_option]

# temp_img = attachments.Image('https://i.groupme.com/250x106.gif.e8fb4a52653f40c0889558e5638753d3.large')
# fillbot.post('', temp_img)

# sendMention(["I can do a lot of things... you just haven't figured them out yet @", targets[0].nickname], targets[0].user_id)
"""
...
run(host='192.168.145.145', port=5001)
